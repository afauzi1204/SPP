...
<body>
  ...
  <table width="100%" border="0">
    <tr>
      <td width="100">Nomor Induk Siswa</td>
      <td width="5">:</td>
      <?php foreach ($siswa as $row) : ?>
    ...
    <tr>
      ...
      <td>:</td>
      <?php foreach ($siswa as $row) : ?>
        <td><?php echo $row['class_name'],' ', $row['majors_name']?></td>
      <?php endforeach; ?>
    </tr>
    ...
    <table width="100%" style="font-size: 10px;" border="1">
      ...
      ?>
        <tr>
          ...
          <td style="padding:0 5px; white-space: nowrap;"><?php echo ($row['bulan_bill']) ? 'Rp. ' . number_format($row['bulan_bill'], 0, ',', '.') : 'Rp. 0' ?></td>
          <td style="padding:0 5px; white-space: nowrap;"><?php echo ($row['bulan_status'] == 0) ? 'Rp. ' . number_format($row['bulan_bill'], 0, ',', '.') : 'Rp. 0' ?></td>
          <!-- <td style="padding:0 5px;"><?php echo ($row['bulan_status'] == 1) ? 'Lunas' : 'Belum Lunas' ?></td> -->
          <td style="padding:0 5px;"><?php echo ($row['bulan_bill']==0 or $row['bulan_status'] == 1) ? 'Lunas' : 'Belum Lunas' ?></td>
        </tr>
      <?php
        $i++;
      endforeach
      ...
      ?>
        <tr>
          ...
          <td style="padding:0 5px;"><?php echo ($row['bebas_bill'] - $row['bebas_total_pay'] != 0) ? 'Rp. ' . number_format($row['bebas_bill'] - $row['bebas_total_pay'], 0, ',', '.') : 'Rp. 0' ?></td>
      ...
      ?>
      <!-- <tr>
          <td colspan="3" align="center">TOTAL KEKURANGAN: </td>
          <td colspan="2" align="center"><?php echo ($row['bebas_bill'] - $row['bebas_total_pay'] != 0) ? 'Rp. ' . number_format($row['bebas_bill'] - $row['bebas_total_pay'], 0, ',', '.') : 'Rp. -' ?></td>
        </tr> -->
