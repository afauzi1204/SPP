...
class Report_set extends CI_Controller
{
	...
	public function report_bill()
	{
		...
		$stu = array();
		$free = array();
		// WAJIB MENAMPILKAN SPP
		if (isset($q['p']) && !empty($q['p']) && $q['p'] != '') {
			$params['period_id'] = $q['p'];
			$param['period_id'] = $q['p'];
			// $stu['period_id'] = $q['p']; //menampilkan pembayaran bulanan
			$free['period_id'] = $q['p'];
		}
		...
		if (isset($q['x']) && !empty($q['x']) && $q['x'] != '') {
			// MENAMPILKAN PER ITEM PEMBAYARAN 
			$params['payment_id'] = $q['x'];
			// $param['payment_id'] = $q['x']; // MENAMPILKAN SELURUH PEMBAYARAN
			$free['payment_id'] = $q['x'];
		}
		$param['payment'] = TRUE;
		$params['grup'] = TRUE;
	...
	public function report()
	{
		...
		foreach ($data['bulan'] as $row) {
			...
			$objSheet->setCellValueExplicit('D' . $cell, $row['class_name'].' '.$row['majors_name'], PHPExcel_Cell_DataType::TYPE_STRING);
		...
		foreach ($data['student'] as $row) {
			if ($row['student_status'] == 1) {
				...
				foreach ($data['free'] as $key) {
					...
					// $objSheet->setCellValue(getCell($alphdata) . $cell, idr_format($data['bulan_bill']));
