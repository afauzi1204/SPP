								...
								<div class="form-group">
									...
									<select class="form-control" name="x">
										<?php foreach ($py as $row) : ?>
											<option <?php echo (isset($q['x']) and $q['x'] == $row['payment_id']) ? 'selected' : '' ?> value="<?php echo $row['payment_id'] ?>"><?php echo $row['pos_name'] . ' - T.P ' . $row['period_start'] . '/' . $row['period_end']; ?></option>
										<?php endforeach; ?>
									</select>
								</div>
							</div>
							...
							<table class="table table-responsive table-hover table-bordered" style="white-space: nowrap;">
								...
								<?php foreach ($student as $row) : ?>
									<?php if ($row['student_status'] == 1) { ?>
										<tr>
											<td><?php echo $no++ ?></td>
											<td><?php echo strtoupper($row['student_full_name']) ?></td>
											<td class="text-center"><?php echo $row['class_name'] . ' ' . $row['majors_name'] ?></td>
											<?php foreach ($free as $key) : ?>
												<?php if ($row['student_student_id'] == $key['student_student_id']) { ?>
													<td style="text-align:center; color:<?php echo ($key['bebas_bill'] == $key['bebas_total_pay']) ? '#00E640' : 'red' ?> ">
											...
											<?php foreach ($bulan as $key) : ?>
												<?php if ($key['student_student_id'] == $row['student_student_id']) { ?>
													<td style="color:<?php echo ($key['bulan_status'] == 1 or $key['bulan_bill'] == 0) ? '#00E640' : 'red' ?>">
														<?php echo ($key['bulan_status'] == 1 or $key['bulan_bill'] == 0)  ? 'Lunas' : number_format($key['bulan_bill'], 0, ',', '.') ?>
													</td>
												<?php } ?>
											<?php endforeach ?>
										</tr>
